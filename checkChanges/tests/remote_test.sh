...
#Remote server for this test will be localhost. We can prepare the folder in local
#while checkChanges script will create/update/remove by ssh/scp
...
bash $checkChangesPath/checkChanges -s "$src_rootdir" -d "$dst_rootdir" -r "$remote_user" -v --color &
pid=$!
sleep 10
#----- STEP 2 -----
